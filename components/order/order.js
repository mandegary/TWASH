...
const Order = (props) => {
    ...
    if (!event.target.validity.valid) {
      ...
      switch (id) {
        ...
        case 7:
          ...
          setCarTag(
            num0 + num1 + num2 + num3 + num4 + num5 + num6 + event.target.value
          );
  ...
  const timesHandler = (newDate) => {
    ...
    if (
      momentJalaali(today).jDate() == momentJalaali(newDate).jDate() &&
  ...
  useEffect(() => {
    ...
    if (!isEmpty(props.orderData)) {
      ...
      else setServicesTitle(_servicesTitle);
      ...
      if (props.orderData.user_cars != undefined) {
        ...
        props.orderData.user_cars.map((car, index) =>
          car.name != ""
            ? _cars.push({ title: car.model.name, id: car.id })
            : null
        );
        setCars(_cars);
        ...
        if (props.orderData.selectedCar > 0) {
          ...
          for (let i = 0; i < props.orderData.user_cars.length; i++)
            if (
              props.orderData.user_cars[i].id == props.orderData.selectedCar
            ) {
              setSelectedCarTitle(props.orderData.user_cars[i].model.name);
              setSelectedCarIsSelected(true);
              if (
                props.orderData.carTag == undefined &&
                props.orderData.user_cars[i].plaque != null
              ) {
              ...
              if (props.orderData.cardImg == undefined)
                setFile(props.orderData.user_cars[i].card_image);
      ...
      if (
        ...
        props.orderData.carTag != undefined &&
        props.orderData.cardImg != undefined
      ) {
      ...
      .then((responseJson) => {
        if (responseJson.message == "ماشین‌ها با موفقیت دریافت شد.") {
          ...
          responseJson.user_cars.map((car, index) =>
            car.name != ""
              ? x.push({ title: car.model.name, id: car.id })
              : null
          );
          setCars(x);
          /*if (!isEmpty(props.orderData)) {
    ...
    if (value == null) {
      ...
      for (let i = 0; i < carBrands.length; i++) {
        if (carBrands[i].name == value) {
          ...
          let result =
            ((cBrand != 0 && carModel != 0) || selectedCar != 0) &&
            date != "" &&
            time != "" &&
            services.length > 0 &&
            isTooOrRooSelected;
  ...
  const carModelHandler = (e, value) => {
    ...
    let result =
      ((carBrand != 0 && cModel != 0) || selectedCar != 0) &&
      date != "" &&
      time != "" &&
      services.length > 0 &&
      isTooOrRooSelected;
  ...
  const selectedCarHandler = (e, value) => {
    ...
    let result =
      ((carBrand != 0 && carModel != 0) || cSelected != 0) &&
      date != "" &&
      time != "" &&
      services.length > 0 &&
      isTooOrRooSelected;
  ...
  const servicesHandler = (event) => {
    ...
    if (_services["rooShoyi"]) {
      ...
      setRooShoyiTooShooyi(false);
      if (toggle) setRooShoyiTooShooyiClass("false disabled");
    ...
    if (_services["rooShoyiTooShooyi"]) {
      ...
      if (_services["rooShoyi"] || _services["tooShooyi"])
        setRooShoyiTooShooyiClass("false disabled");
      else if (toggle) setRooShoyiTooShooyiClass("false disabled");
      else setRooShoyiTooShooyiClass("false");
      if (!_services["rooShoyi"]) {
        setRooShoyi(false);
      ...
      if (!_services["tooShooyi"]) {
        if (toggle) setTooShooyiClass("false disabled");
        else setTooShooyiClass("false");
        setTooShooyi(false);
        _tooShooyi = false;
        removeItemOnce(_selectedServices, 2);
        removeItemOnce(_selectedServicesTitle, "توشویی");
    ...
    if (_services["dashboardWax"]) {
      ...
      setDashboardWax(false);
      if (toggle) setDashboardWaxClass("false disabled");
    ...
    let result =
      ((carBrand != 0 && carModel != 0) || selectedCar != 0) &&
      date != "" &&
      time != "" &&
      _selectedServices.length > 0 &&
      (_rooShoyi || _tooShooyi);
    validate(result);
  };
  ...
  const subscribeModal = () => {
    ...
    if (file != "" && carTag != "" && file != null && carTag != null) {
      ...
      setShowCarItems(true);
      setEditCar(false);
    } else if (carTag == "" || carTag == null)
      Notiflix.Notify.Failure("لطفا پلاک ماشین خود را وارد کنید.");
    else if (file == "" || file == null)
      Notiflix.Notify.Failure("لطفا تصویر کارت ماشین خود را آپلود کنید.");
  };
  ...
  const imageHandler = async (event) => {
    ...
    let _file = new File([event.target.files[0]], _name, {
      type: "image/jpeg",
    });
    //setFile(_file)
    setOrderData({
    ...
    <MuiThemeProvider theme={theme}>
      <div className="orderForm" dir="rtl">
        <Container>
              ...
              {ordersCount > 0 ? (
                <form className="switch-field">
                  <input
                    ...
                    checked={!toggle}
                  />
                  <label htmlFor="switch_right" className="switch_right">
                    خیر
                  </label>
                  <input
                    type="radio"
                    ...
                    checked={toggle}
                  />
                  <label htmlFor="switch_left" className="switch_left">
                    بله
                  </label>
          ...
          <Row
            className="orderRow"
            style={{ paddingTop: "0", margin: "0 20px" }}
          >
            ...
            <Col xl={3} lg={3} md={12} sm={12} xs={12}>
              {loadingPrice ? (
                ...
                <label className="servicesPrice">
                  هزینه خدمات : {addCommas(price)} تومان
                </label>
              )}
            </Col>
          </Row>
            ...
            <Col xl={4} lg={10} md={12} sm={12} xs={12}>
              {time != "" && timeEnd != "" && (
                <label className="servicesTime">
                  خودرو شما در بازه زمانی {time} تا {timeEnd} شسته خواهد شد.
                </label>
              )}
            </Col>
          </Row>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            لطفا برای استفاده از خدمات درغیاب مشتری به نکات زیر توجه داشته
            باشید:
            <br />
            - محل پارک کردن ماشین باید به گونه ای باشد که دسترسی به همه نقاط
            ماشین امکان پذیر باشد (کنار دیوار یا چسبیده به جدول و ... نباشد،
            نقاطی که دست کارواشمن نرسد شسته نخواهد شد.)
            <br />
            - هم چنین داخل شیشه ها دستمال کشیده نخواهد شد.
            <br />
            برای حفظ امنیت در استفاده از خدمات کارواش در غیاب مشتری لطفا پلاک
            خودرو خود را وارد کرده و کارت ماشین را آپلود نمایید.
          </DialogContentText>
          <MuiThemeProvider theme={theme}>
            <div dir="rtl">
              ...
              <div className="uploadInput">
                ...
                <span className="inputFileLabel">
                  ...
                  <label htmlFor="file">آپلود تصویر کارت ماشین</label>
                </span>
                <img src={file} className="carImg" />
              </div>
            </div>
